        ...
        public ActionResult Create([Bind(Include = "SliderId,Baslik,Aciklama,ResimURL")] Slider slider,HttpPostedFileBase ResimURL)
        {
            if (ModelState.IsValid)
            {
                if (ResimURL != null)
                {
                    ...
                    img.Resize(1024, 360); //Resimin yükseklik ve genişlik değeri ayalandı
                    img.Save("~/Uploads/Slider/" + sliderimgname); //resim dosyasını olşturduğumuz klasör içine kayıt ettik
